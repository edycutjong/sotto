...
export default function SottoDashboard() {
  ...
  const connectDemoWallet = () => {
    ...
    setTimeout(() => {
      ...
      setLogs((prev) => [
        ...prev,
        `[Sandbox] Demo identity active: GB_BUYER... (simulation only)`,
      ]);
    }, 600);
  };
  ...
  const [bids, setBids] = useState<Bid[]>([
    {
      ...
      commitment_hash:
        "7e662d8f7a3935e5a39e81ab3d0adbc1bdcde05bae50ffe37aa61217e0c902cb",
    ...
    },
    {
      ...
      commitment_hash:
        "1aaa23d2c902c1fb88594ec9090ca24f8e6fa82f2052050f332fb2abfdf67db2",
  ...
  const [logs, setLogs] = useState<string[]>([
    "[system] Sotto auction initialized. Waiting for bidding phase to close.",
  ]);
  ...
  const handleCreateAuction = (e: React.FormEvent) => {
    ...
    setRebateAmount(0);
    setLogs([
      `[system] New auction created. Budget: $${budgetVal.toLocaleString()} USDC. Reserve: $${reserveVal.toLocaleString()} USDC. Deadline sequence: ${ledgerSequence + deadlineOffset}`,
    ]);
  };
  ...
  const handleSubmission = (e: React.FormEvent) => {
    ...
    setLogs([
      ...logs,
      `[system] Bid commitment submitted by ${bidderAddress}. Hash: 0x${hash.slice(0, 10)}...`,
    ]);
  ...
  const runProverAndSettle = async () => {
    ...
    setLogs((prev) => [
      ...prev,
      `[system] Bidding phase ended (Current: ${ledgerSequence} > Deadline: ${deadline}).`,
    ]);
    ...
    setLogs((prev) => [
      ...prev,
      `[prover] Fetching all public commitments from SottoAuction contract...`,
    ]);
    await delay(600);
    setLogs((prev) => [
      ...prev,
      `[prover] Found ${bids.length} registered commitments.`,
    ]);
    await delay(800);
    setLogs((prev) => [
      ...prev,
      `[prover] Loading private witness variables...`,
    ]);
    await delay(1000);
    ...
    if (!winner) {
      setLogs((prev) => [
        ...prev,
        `[prover] Error: No valid bids found within reserve and budget range.`,
      ]);
      setIsProving(false);
      return;
    }
    ...
    setLogs((prev) => [
      ...prev,
      `[prover] Running range proof checks for candidate bid...`,
    ]);
    await delay(800);
    setLogs((prev) => [
      ...prev,
      `[prover] Range verification: $${win.amount?.toLocaleString()} USDC >= $${reservePrice.toLocaleString()} reserve ... PASS`,
    ]);
    setLogs((prev) => [
      ...prev,
      `[prover] Range verification: $${win.amount?.toLocaleString()} USDC <= $${maxBudget.toLocaleString()} budget ... PASS`,
    ]);
    await delay(800);
    setLogs((prev) => [
      ...prev,
    ...
    setLogs((prev) => [
      ...prev,
      `[prover] Minimum check: Asserting winner bid is lower than or equal to all other commitments...`,
    ]);
    ...
    for (const b of bids) {
      if (b.id !== win.id) {
        setLogs((prev) => [
          ...prev,
          `[prover] Compare: Winner ($${win.amount?.toLocaleString()}) <= Supplier ${b.bidder_address} ... PASS`,
        ]);
        await delay(500);
      }
    }
    ...
    setLogs((prev) => [
      ...prev,
      `[system] ZK Groth16 proof compiled successfully in 4,120ms.`,
    ]);
    await delay(800);
    setLogs((prev) => [
      ...prev,
      `[system] Submitting settle(proof, winner_index: ${bids.indexOf(win)}) to Soroban...`,
    ]);
    await delay(1200);
    ...
    const finalPayout =
      secondLowest === maxBudget ? win.amount || 0 : secondLowest;
    ...
    if (!sandboxMode) {
      try {
        ...
        setLogs((prev) => [
          ...prev,
          `[Stellar] RPC Connected. Auction: ${contractId} | Verifier: ${verifierId}`,
        ]);

        let winnerStellarAddress = win.bidder_address;
        if (
          !winnerStellarAddress.startsWith("G") ||
          winnerStellarAddress.length !== 56
        ) {
          winnerStellarAddress =
            "GCOB3XOYF2E64VS6HNEO3NS3J34J7VWLX7P73L5J5Z6R4M5Y3H2R7OW";
        }
        ...
        if (!connection.isConnected) {
          throw new Error(
            "Freighter wallet not detected. Install the Freighter browser extension (or use Sandbox mode) to settle on Testnet.",
          );
        }
        ...
        setLogs((prev) => [
          ...prev,
          `[Freighter] Requesting wallet signature for Sotto settlement...`,
        ]);
        ...
        if (txStatus.status === "SUCCESS") {
          setLogs((prev) => [
            ...prev,
            `[Stellar] Settle transaction committed! Hash: ${sendResponse.hash}`,
          ]);
        } else {
        ...
        setLogs((prev) => [
          ...prev,
          `[ERROR] Testnet transaction failed: ${errMsg}`,
        ]);
        setIsProving(false);
        return;
      }
    ...
    setLogs((prev) => [
      ...prev,
      `[contract] Verifier verifications passed natively on Protocol 25 BN254 host fn.`,
    ]);
    setLogs((prev) => [
      ...prev,
      `[contract] Payout of $${finalPayout.toLocaleString()} USDC disbursed to Winner (${win.bidder_address}).`,
    ]);
    setLogs((prev) => [
      ...prev,
      `[contract] Vickrey remainder rebate of $${rebate.toLocaleString()} USDC returned to Manufacturer.`,
    ]);
    setLogs((prev) => [
      ...prev,
      `[system] Sotto auction settled successfully.`,
    ]);
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              onClick={() => {
                ...
                setLogs((prev) => [
                  ...prev,
                  `[ledger] Block progress: ledger sequence advanced to ${ledgerSequence + step}.`,
                ]);
              }}
            ...
            {walletConnected ? (
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="text-xs font-semibold px-4 py-2 rounded-lg border bg-slate-900 hover:bg-slate-800 border-slate-700/80 hover:border-slate-600 text-white shadow-sm transition-all cursor-pointer"
                >
                ...
                <button
                  ...
                  className="text-xs font-semibold px-3 py-2 rounded-lg border border-amber-500/30 bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 transition-all cursor-pointer"
                >
                  DEMO
                </button>
              </div>
          ...
          <div className="glass-panel p-8 relative overflow-hidden flex flex-col justify-between min-h-[260px]">
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <span className="font-mono text-xs tracking-wider text-violet-400 uppercase">
                  ACTIVE ESCROW ROOM
                </span>
                <span
              ...
              <p className="text-slate-400 text-sm max-w-xl">
                Sealed-bid reverse Vickrey auction. Bids remain
                cryptographically sealed until sequence {deadline}. Winning bid
                proves lowest spot without revealing competitors.
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-6 gap-6 pt-6 border-t border-slate-800/80 mt-6 font-mono">
              ...
              <div>
                ...
                <span className="text-sm sm:text-base font-bold text-white">
                  ${reservePrice.toLocaleString()} USDC
                </span>
              </div>
              <div>
                <span className="text-[11px] text-slate-500 block uppercase">
              ...
              <div>
                ...
                <span
                  className={`text-sm sm:text-base font-bold ${biddingClosed ? "text-red-400" : "text-amber-500"}`}
                >
                  {biddingClosed
                    ? "0 (Closed)"
                    : `${(deadline - ledgerSequence).toLocaleString()}`}
                </span>
              </div>
              <div>
                <span className="text-[11px] text-slate-500 block uppercase">
                ...
                <span
                  className="text-xs font-bold text-white block truncate w-24"
                  title={tokenAddress}
                >
                  {tokenAddress}
                </span>
                          ...
                          {b.revealed ? (
                            ...
                            <div className="flex items-center gap-2 text-slate-500 text-xs bg-slate-950/80 border border-slate-850 px-3 py-1.5 rounded-lg">
                              <svg
                                ...
                                className="h-4.5 w-4.5 text-amber-500/80"
                                fill="none"
                                viewBox="0 0 24 24"
                                stroke="currentColor"
                              >
                    ...
                    <div className="mt-4 p-5 bg-amber-500/5 border border-amber-500/20 rounded-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
                      <div>
                        <div className="text-sm font-semibold text-amber-400">
                          Deadline Block sequence passed!
                        </div>
                        <div className="text-xs text-slate-400 mt-1">
                          Winning bidder can now compile the Lowest Bid Groth16
                          proof to claim escrow payout.
                        </div>
                      </div>
                      <button
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4 text-xs font-mono">
                        ...
                        <div className="bg-slate-950/60 p-3 rounded-lg border border-slate-900">
                          ...
                          <span className="text-emerald-400 font-semibold mt-1 block">
                            ${winningAmount.toLocaleString()} USDC
                          </span>
                        </div>
                        <div className="bg-slate-950/60 p-3 rounded-lg border border-slate-900">
                          <span className="text-slate-500 block">
                          ...
                          <span className="text-violet-400 font-semibold mt-1 block">
                            ${rebateAmount.toLocaleString()} USDC
                          </span>
                        </div>
                      </div>
                    </div>
          ...
          <div className="glass-panel p-6 flex flex-col min-h-[300px] bg-slate-950/40">
            <div className="flex items-center justify-between mb-4 border-b border-slate-800/80 pb-3">
              ...
              <div className="flex items-center gap-1.5">
                <span
                  className={`w-2 h-2 rounded-full ${isProving ? "bg-amber-500 animate-pulse" : "bg-slate-700"}`}
                ></span>
                <span className="text-[10px] font-mono text-slate-500">
                  {isProving ? "COMPILING" : "IDLE"}
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3 font-mono text-[10px]">
              <div className="bg-black/40 border border-slate-900 p-3.5 rounded-lg flex flex-col gap-2">
                ...
                <span className="text-slate-400 font-semibold">
                  SELECT auction_id, bidder_address, commitment_hash,
                  encrypted_bid FROM sotto_auction_bids WHERE auction_id = 101;
                </span>
              </div>
              ...
              {bids.map((b) => (
                <div
                  key={b.id}
                  className="bg-slate-900/60 p-3 rounded-lg border border-slate-850 flex flex-col gap-1.5"
                >
                  <div className="flex justify-between text-slate-400 font-bold">
                    <span>Bidder: {b.bidder_address}</span>
                    <span className="text-[9px] text-slate-500">
                      {b.revealed ? "Winner - Revealed" : "Shielded"}
                    </span>
                  </div>
                  <div className="text-slate-500 break-all">
                    Commitment:{" "}
                    <span className="text-slate-400">
                      0x{b.commitment_hash.slice(0, 16)}...
                    </span>
                  </div>
