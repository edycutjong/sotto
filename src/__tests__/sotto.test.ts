...
import { toField, serializeProof, serializePublicInputs, SottoProver } from '../lib/zkProver';

// Mock snarkjs module
jest.mock('snarkjs', () => ({
  wtns: {
    calculate: jest.fn().mockResolvedValue({}),
    exportJson: jest.fn().mockResolvedValue([0, '123456789012345', '234567890123456', '345678901234567']),
  },
  groth16: {
...
function computeWinnerAndRebate(
  ...
  for (const b of bids) {
    if (b.amount >= reservePrice && b.amount <= maxBudget && b.amount < minBid) {
      minBid = b.amount;
      winner = b;
    }
  }
  ...
  for (const b of bids) {
    if (b.bidder !== winner.bidder && b.amount >= reservePrice && b.amount < secondLowest) {
...
describe('Sotto Sealed-Bid Cryptographic & Auction Mechanics', () => {
  ...
  it('should verify commitment hashing matches poseidon2_mock implementation', () => {
    ...
    expect(hash).toBe('fb1a1fd789ef33fd20e6f4dc2a7b6c18974a2d1bf77d045ebe01fae705ebaa65');
  });
