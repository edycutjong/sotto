...
test.describe("Sotto Sealed-Bid Settlement Flow E2E", () => {
  test("should allow navigating to submission tab and registering a new bid commitment", async ({ page }) => {
    await page.goto("/");
    
    // Switch to submit bid tab
    const submitTab = page.getByRole("button", { name: "Submit Bid Commitment" });
    ...
    // Click submit
    const submitButton = page.getByRole("button", { name: "Generate Hash & Submit Bid" });
