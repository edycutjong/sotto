...
const VERIFIER =
  process.env.RESERVE_VERIFIER_ID ||
  "CA3DTYCC77WMKD7Y43T7TR6SCO5OZVO5FSMF2ALSX5KK7PNQRQ3UQA6W";
...
async function run() {
  ...
  const reserve_price = "40000",
    reserve_salt = "12345";
  const winning_bid = "60000",
    max_budget = "100000",
    round_number = "5",
    dutch_decrement = "10000";

  const poseidon = await buildPoseidon();
  const reserve_commitment = poseidon.F.toObject(
    poseidon([BigInt(reserve_price), BigInt(reserve_salt)]),
  ).toString();
  ...
  const { proof, publicSignals } = await snarkjs.groth16.fullProve(
    input,
    `${C}/reserve_proof_js/reserve_proof.wasm`,
    `${C}/r_final.zkey`,
  );
  const vk = (await import(`${C}/reserve_vk.json`, { with: { type: "json" } }))
    .default;
  console.log(
    "off-chain verify:",
